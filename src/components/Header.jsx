...
const Header = ({ children }) => {
    ...
    const [navSize, setNavSize] = useState(
        pathRoot ? HEADER_HEIGHT_ROOT : HEADER_HEIGHT_NORMAL
    )
    const [navColor, setNavColor] = useState(
        pathRoot ? HEADER_BACKGROUND_TRANSPARENT : HEADER_BACKGROUND_NORMAL
    )
    ...
    return (
        <header
            style={headerStyle}
            className={`flex flex-wrap justify-between ${background} place-items-center fixed top-0 right-0 left-0 z-50`}
        >
            <Button
                asChild
                variant='link'
            ...
            >
                <Link
                    ...
                    className='h-fit p-3 flex items-center ml-1 md:ml-5 hover:opacity-80'
                >
            ...
            {isDropdownOpen && (
                <div
                    className={`w-screen h-screen bg-muted justify-start flex flex-col gap-4 px-4`}
                    id='hamburger-menu'
                >
                    {children}
